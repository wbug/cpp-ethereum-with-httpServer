...
namespace
{
std::string dumpStackMemoryStorage(ExtVM const& _ext, VM& _vm)
{
    ostringstream o;
    o << "\n    STACK\n";
    ...
    if (!m_t.hasZeroSignature())
    {
        ...
        if (m_s.balance(m_t.sender()) < totalCost)
        {
            BOOST_LOG(m_execLogger) << "Not enough cash: Require >" << totalCost << "=" << m_t.gas()
                ...
                << m_s.balance(m_t.sender()) << " for sender: " << m_t.sender();            m_excepted = TransactionException::NotEnoughCash;
...
OnOpFunc Executive::simpleTrace()
{
    ...
    return [&traceLogger](uint64_t steps, uint64_t PC, Instruction inst, bigint newMemSize,
               bigint gasCost, bigint gas, VM* voidVM, ExtVMFace const* voidExt) {
        ExtVM const& ext = *static_cast<ExtVM const*>(voidExt);
        VM& vm = *voidVM;

        BOOST_LOG(traceLogger) << dumpStackMemoryStorage(ext, vm);
